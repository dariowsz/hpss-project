...
for lambda_val in [0.05, 0.1, 0.2, 0.5]:
    gen_harmonic, sr = librosa.load(
        f"../outputs/3/{track}_mix/harmonic_lambda_{lambda_val}.wav",
        sr=22050,
        mono=True,
    )
    gen_percussion, sr = librosa.load(
        f"../outputs/3/{track}_mix/percussive_lambda_{lambda_val}.wav",
        sr=22050,
        mono=True,
    )
...
output_table_path = f"../evaluation/{track}_mix/3_experiment_eval.csv"
